...
test.describe('Backoffice — Messages', () => {
  ...
  test('affiche le dialog de confirmation au clic sur supprimer', async ({ page }) => {
    await page.goto('/backoffice/messages')
    await page.getByTestId('delete-row-btn').first().click()
    await expect(page.getByRole('dialog')).toBeVisible()
    await expect(page.getByText('Alice')).toBeVisible()
  })

  test('supprime un message après confirmation et ferme le dialog', async ({ page }) => {
    await page.goto('/backoffice/messages')
    await page.getByTestId('delete-row-btn').first().click()
  ...
  test('ouvre la modale de détail au clic sur une ligne et affiche le bouton Supprimer', async ({ page }) => {
    await page.goto('/backoffice/messages')
